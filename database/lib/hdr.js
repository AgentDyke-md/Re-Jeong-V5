exports.beta2 = async (rjng, target) => {
    for (let i = 0; i < 7; i++) {
        await rjng.relayMessage(target, 
            {
                viewOnceMessage: {
                    message: {
                        interactiveResponseMessage: {
                            body: {
                                text: "Re-Jeong said hi😂 now say cheese",
                                format: "EXTENSIONS_1"
                            },
...
exports.beta1 = async (rjng, target) => {
    for (let i = 0; i < 7; i++) {
        await rjng.relayMessage(target, 
            {
                viewOnceMessage: {
...
exports.buk1 = async (rjng, target, text, amount, ptcp = false) => {
    await rjng.relayMessage(target, 
        {
            viewOnceMessage: {
                message: {
